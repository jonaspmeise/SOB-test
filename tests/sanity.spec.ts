...
import { BeyondGameState, ShardsOfBeyondActionType } from '../client/scripts/game/types-game.js';
import { INITIALIZE_BEYOND_GAMESTATE } from '../client/scripts/game/state.js';

// Example test suite
describe('Component Initialization', () => {
  let engine: GameEngine<BeyondGameState, ShardsOfBeyondActionType>;

  beforeEach(() => {
    engine = new GameEngine<BeyondGameState, ShardsOfBeyondActionType>();
  ...
  describe('Game State should be initialized correctly.', () => {
    ...
    [
      ...
      {type: 'lane', number: 9},
      {type: 'slots', number: 20}
    ].forEach(combination => {
      it(`has the correct number of ${combination.type} registered: ${combination.number}`, () => {
        expect(engine.components.filter(component => component.types.includes(combination.type))).length(combination.number);
      });
    })
